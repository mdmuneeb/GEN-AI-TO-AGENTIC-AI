...
# -------------------------
def check_performance_scores(state: AuditState) -> Literal["OptimizedReport", "PerformanceAnalyzer"]:
    """
    Check if mobile and desktop performance scores are both > 90.
    Returns a dict with a flag that determines the next path.
    """
    page_data = state.get("page_data", {})
    mobile_score = page_data.get("mobile", {}).get("scores", {}).get("performance_score", 0)
    desktop_score = page_data.get("desktop", {}).get("scores", {}).get("performance_score", 0)

    if mobile_score > 90 and desktop_score > 90:
        return "OptimizedReport"  # Skip audit, directly write optimized report
    else:
        return "PerformanceAnalyzer" # Run the full workflow


# -------------------------
# Conditional Final Report Node
# -------------------------
def optimized_report(state: AuditState):
    """
    Return a simple message if optimization is not required.
    """
    return {"final_report": "Website optimization is not required. Both mobile and desktop performance scores are above 90."}
...
builder.add_node("OptimizedReport", optimized_report)
...
# Conditional Node -> Either full audit or optimized report
builder.add_conditional_edges(
    "PageInsightsAnalyzer",
    check_performance_scores,
    # {   # if Literal not labelled then map check_performance_scores output
    #     "OptimizedReport": "OptimizedReport",    
    #     "PerformanceAnalyzer": "PerformanceAnalyzer"
    # }
)

# Full audit parallel workflow 
builder.add_edge("PerformanceAnalyzer", "IssuePrioritizer")
builder.add_edge("IssuePrioritizer", "AuditSummaryGenerator")
builder.add_edge("IssuePrioritizer", "ActionRecommender")
builder.add_edge("AuditSummaryGenerator", "FinalReport")
builder.add_edge("ActionRecommender", "FinalReport")
builder.add_edge("FinalReport", END)

# Optimized path goes straight to END
builder.add_edge("OptimizedReport", END)
